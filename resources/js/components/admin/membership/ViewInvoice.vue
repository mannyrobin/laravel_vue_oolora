<template>
<div class="panel-wrap has-header">

	<div class="panel-header px-3 bg-white border-bottom d-flex justify-content-between align-items-center">
		<h6 class="mb-0 cursor-pointer" @click="closePanel"><i class="far fa-long-arrow-left"></i> View Invoice</h6>
		<div>
			<button class="btn btn-sm btn-light" v-print="'#printMe'"><i class="fal fa-print fa-fw"></i> Print</button>
			<a :href="$appSettings.appUrl + '/pdf/invoice/' + invoice.id" class="btn btn-sm btn-light"><i class="fal fa-download fa-fw"></i> Download</a>
		</div>
	</div>

	<div class="panel-body p-3 bg-gray">
...
export default {
    ...
    methods: {
    	getInvoice () {
            ...
            axios.get('admin/payments/' + this.invoiceId)
            .then(response => {
