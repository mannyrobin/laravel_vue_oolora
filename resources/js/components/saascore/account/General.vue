...
<div>
    ...
    <div class="row mb-4">
        <div class="col-md-6 order-md-2">
            ...
            <div class="row mb-4 align-items-center justify-content-center ml-md-5">
                <div class="col-4">

                    <img :src="userData.avatar" class="img-fluid rounded-circle">
                ...
                <div class="col-8">
                    ...
                    <file-upload
                        ...
                        <button-spinner :loading="btnLoadingAvator" class="btn-light btn-sm">Change Avatar</button-spinner>
                        <div slot="tip" class="text-gray-600 font-size-sm">jpg/png files with a size less than 2MB</div>
                    </file-upload>
        ...
        <div class="col-md-6 order-md-1">
            ...
            <form @submit.prevent="updateUser()" data-vv-scope="form-account">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name"
                        :class="['form-control', { 'is-invalid': errors.has('form-account.name') }]"
                        v-validate="'required'"
                        v-model="userData.name">
                    <div class="invalid-feedback" v-show="errors.has('form-account.name')">{{ errors.first('form-account.name') }}</div>
                </div>

                <div class="form-group">
                    <label>Email Address</label>
                    <input type="email" name="email"
                        :class="['form-control', { 'is-invalid': errors.has('form-account.email') }]"
                        v-validate="'required|email'"
                        v-model="userData.email">
                    <div class="invalid-feedback" v-show="errors.has('form-account.email')">{{ errors.first('form-account.email') }}</div>
                </div>


                <div class="mt-5 font-weight-semibold font-size-lg">Preferences</div>
                <p class="text-gray-600">Manage and configure your account preferences</p>

                <div class="form-group">
                    <label>Timezone</label>
                    <form-select name="network" v-model="userData.timezone">
                        <select-option
    ...
    <div class="bg-light p-2 m-n3 border-top text-center">
        <button-spinner :loading="btnLoadingGeneral" class="btn-primary" @click.prevent="updateUser()">Save Settings</button-spinner>
    </div>
...
export default {
    ...
    data () {
        return {
            btnLoadingGeneral: false,
            btnLoadingAvator: false,
    ...
    methods: {
        updateUser () {
            this.$validator.validateAll('form-account').then((success) => {

                // Return if validation failed
                if ( ! success )
                    return

                this.btnLoadingGeneral = true;

                axios.put('user/'+ this.userData.id, this.userData)
                .then(response => {
                    this.btnLoadingGeneral = false;
                    this.$alert.success(response.data.message);

                    // Update user global variables
                    window.appUser.name = this.userData.name;
                })
                .catch(error => {
                    this.btnLoadingGeneral = false;
                    this.$backendErrors(error.response.data, 'form-account');
        ...
        avatarUploadSuccess (result) {
            this.btnLoadingAvator = false;

            this.$alert.success(result.message);
            this.userData.avatar = result.avatar;

            // Update user global variables
            window.appUser.avatar = result.avatar;
        },
